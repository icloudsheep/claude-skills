...
## 8. 平台会话 ID 不可用时所有记录落到 `Anon-0000`

**症状**
在手动 shell 或平台未暴露会话变量时运行，记录都归入 `Anon-0000`。

**根因**
脚本优先读取 `AILOG_SESSION_ID`，其次读取 Codex 的 `CODEX_THREAD_ID` 或 Claude Code 的 `CLAUDE_CODE_SESSION_ID`，再确定性派生代号。没有稳定 ID 时刻意使用固定匿名代号，避免随机值把同一会话拆散。

**解法**
- Claude Code 与 Codex 正常会话通常自动提供 ID，可用 `--status` 查看 `runtime.session_id`
- 手动 shell 中使用 `--session-id <稳定值>` 或 `export AILOG_SESSION_ID=<稳定值>`
...
**根因**
Claude Code 与 Codex 都可能压缩上下文，早期对话可能被摘要或滚出窗口。凭当前上下文划分时只能看到仍可用的内容。

**解法**
如果一段会话很长且经历过压缩，选**"读 transcript"**数据源。先用 `--status` 确认 `runtime.transcript`；Claude Code 默认搜索 `~/.claude/projects`，Codex 默认搜索 `${CODEX_HOME:-~/.codex}/sessions`。也可用 `--transcript` 显式指定。代价是 token 消耗大。
